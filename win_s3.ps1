...
# Optional attributes
$rm = Get-Attr -obj $params -name rm -default $False
$overwrite = Get-Attr -obj $params -name overwrite -default $False
...
ElseIf ($method -eq "download"){
    ...
    If (-Not ($key[$key.length-1] -eq "/" -Or $key[$key.length-1] -eq "\")){
        Try{
            Read-S3Object -BucketName $bucket -Key $key -File (Join-Path -path $local -ChildPath $key)
            $result.changed = $true
        }
        Catch {
...
Set-Attr $result "bucket" $bucket.toString()
Set-Attr $result "key" $key.toString()
Set-Attr $result "method" $method.toString()
...
Set-Attr $result "local" $local.toString()
Set-Attr $result "rm" $rm.toString()
